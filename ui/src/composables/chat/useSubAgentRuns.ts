...
function asTraceEvent(value: unknown): SubAgentTraceEvent | null {
  ...
  if (!event.invocationId || !event.eventId || !event.eventType || event.sequence === undefined)
    return null
...
/**
 * 将实时自定义事件协议与常规 AG-UI 消息缓冲区分离。
 * 调用 ID 用于标识单个卡片；事件 ID 和单调递增的序列号
 * 确保重连重放以及并发兄弟代理的幂等性。
 */
export function useSubAgentRuns() {
  ...
  function accept(event: SubAgentTraceEvent) {
    ...
    const nextEvents = [...(existing?.events ?? []), event].sort(
