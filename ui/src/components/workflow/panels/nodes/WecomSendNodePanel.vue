    ...
    <AForm layout="vertical">
      ...
      <PanelSection title="节点配置">
        <AFormItem required>
          <template #label>
            企业微信渠道
            <ATooltip title="选择已配置并启用的企业微信机器人渠道">
        ...
        <AFormItem>
          <template #label>
            @用户ID
            <ATooltip title="需要 @的企业微信用户ID，多个用逗号分隔。可选，支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
            :model-value="String(node.data.config?.mentionUsers || '')"
            placeholder="user1, user2"
            @update:model-value="(v: any) => updateConfig('mentionUsers', v)"
          />
        </AFormItem>
        ...
        <AFormItem>
          ...
          <BlurInput
            :model-value="String(node.data.config?.mentionMobiles || '')"
            placeholder="138xxxx, 139xxxx"
            @update:model-value="(v: any) => updateConfig('mentionMobiles', v)"
          />
        </AFormItem>
