    ...
    <AForm layout="vertical">
      ...
      <PanelSection title="节点配置">
        <AFormItem required>
          <template #label>
            钉钉渠道
            <ATooltip title="选择已配置并启用的钉钉机器人渠道">
        ...
        <AFormItem>
          <template #label>
            消息标题
            <ATooltip title="消息标题。支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
        ...
        <AFormItem>
          <template #label>
            @手机号
            <ATooltip title="需要 @的用户手机号，多个用逗号分隔。可选，支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
            :model-value="String(node.data.config?.atMobiles || '')"
            placeholder="138xxxx, 139xxxx"
            @update:model-value="(v: any) => updateConfig('atMobiles', v)"
          />
        </AFormItem>
        ...
        <AFormItem>
          <template #label>
            @用户ID
            <ATooltip title="需要 @的钉钉用户ID，多个用逗号分隔。可选，支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
            :model-value="String(node.data.config?.atUserIds || '')"
            placeholder="user1, user2"
            @update:model-value="(v: any) => updateConfig('atUserIds', v)"
          />
        </AFormItem>

        <div class="config-row">
          <span class="config-row-label">@所有人</span>
          <ASwitch
            :checked="Boolean(node.data.config?.isAtAll)"
            @update:checked="(v: any) => updateConfig('isAtAll', v)"
          />
        </div>
        ...
        <div class="message-field">
          ...
          <ConfigCodeEditor
            ...
            language="txt"
            :placeholder="messageMode === 'template' ? '使用 ${输入绑定名} 引用输入绑定' : '直接输入消息内容'"
            :maximize-target="panelRoot"
            @maximize-change="onEditorMaximizeChange"
          />

          <span class="field-help">选择「模板渲染」后，内容经所选模板格式渲染再发送；标题和 @用户同样使用该格式渲染。</span>
        </div>
      </PanelSection>
...
.config-row { display: flex; align-items: center; justify-content: space-between; min-height: 32px; margin-bottom: 16px; }
.config-row-label { flex-shrink: 0; font-size: 14px; color: rgba(0, 0, 0, 0.88); }
