    ...
    <AForm layout="vertical">
      ...
      <PanelSection title="节点配置">
        <AFormItem required>
          <template #label>
            邮箱渠道
            <ATooltip title="选择已配置并启用的邮箱渠道（SMTP）">
        ...
        <AFormItem required>
          <template #label>
            收件人
            <ATooltip title="收件人邮箱地址，多个用逗号分隔。支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
            :model-value="String(node.data.config?.toRecipients || '')"
            placeholder="user1@example.com, user2@example.com"
            @update:model-value="(v: any) => updateConfig('toRecipients', v)"
          />
        </AFormItem>
        ...
        <AFormItem>
          <template #label>
            抄送
            <ATooltip title="抄送邮箱地址，多个用逗号分隔。可选，支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
            :model-value="String(node.data.config?.ccRecipients || '')"
            placeholder="cc@example.com"
            @update:model-value="(v: any) => updateConfig('ccRecipients', v)"
          />
        </AFormItem>
        ...
        <AFormItem>
          <template #label>
            邮件主题
            <ATooltip title="邮件主题。支持 ${输入绑定名} 模板语法">
              <QuestionCircleOutlined class="help-icon" />
            </ATooltip>
          </template>
          <BlurInput
        ...
        <div class="message-field">
          <div class="message-header">
            <span class="form-label required-field">邮件内容</span>
