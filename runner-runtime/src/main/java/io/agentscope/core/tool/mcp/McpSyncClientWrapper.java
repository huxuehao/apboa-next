...
// Apboa 同包路径源码覆盖（来源：agentscope-core 1.0.12 sources jar，升级时须 diff 同步）：
// callTool 增加从 Reactor Context 读取 _meta 并构造三参 CallToolRequest 的能力，
// 用于工具调用注入平台身份断言（docs/identity-propagation-design.md §6.M3）。
// 除标注"Apboa 覆盖点"的段落外，其余内容与原版一致。
...
public class McpSyncClientWrapper extends McpClientWrapper {
    ...
    public Mono<McpSchema.CallToolResult> callTool(String toolName, Map<String, Object> arguments) {
        ...
        // Apboa 覆盖点：经 Reactor Context 接收调用方（LazyMcpAgentTool）写入的 _meta
        // （身份断言等），非空时构造三参 CallToolRequest 随请求下发；无 Context 时行为与原版一致
        return Mono.deferContextual(
                        ctxView -> {
                            Map<String, Object> meta = ctxView.getOrDefault(
