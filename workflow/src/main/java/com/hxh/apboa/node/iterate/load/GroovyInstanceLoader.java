 ...
 * @author huxuehao
 **/
@Component("iterateGroovyInstanceLoader")
public class GroovyInstanceLoader implements InstanceLoader {
    ...
    @Override
    public IteratorExecutor loadInstance(String codeSource) {
        if (FuncUtils.isNotEmpty(codeSource)) {
            ...
            if (!(instance instanceof IteratorExecutor)) {
                throw new IllegalArgumentException("loadNewInstance 执行失败, Glue 脚本类需要继承 " + IteratorExecutor.class.getName());
            }
            return (IteratorExecutor) instance;
        }
