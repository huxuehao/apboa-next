...
import com.hxh.apboa.node.channel.sender.DingTalkSender;
 ...
 * @author huxuehao
 */
public class DingTalkSendNode extends EnhancedNode {

    @Getter
    private final Config config;
    private final TemplateFormatter formatter;

    public DingTalkSendNode(String id, String name, Config config) {
        super(id, name, NodeType.DINGTALK_SEND);
        this.config = config;
    ...
    private NodeOutput successNodeOutput(Map<String, Object> inputs, NodeOutput output) throws Exception {
        ...
        String resolvedAtMobiles = resolveTemplate(config.getAtMobiles(), inputs);
        String resolvedAtUserIds = resolveTemplate(config.getAtUserIds(), inputs);

        ChannelSender sender = new DingTalkSender();
        Map<String, Object> extras = new HashMap<>();
        extras.put("atMobiles", resolvedAtMobiles);
