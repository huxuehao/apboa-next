...
DROP TABLE IF EXISTS `chat_subagent_run`;
CREATE TABLE `chat_subagent_run` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `tenant_id` bigint NOT NULL,
  `session_id` bigint NOT NULL,
  `invocation_id` varchar(128) NOT NULL,
  `parent_invocation_id` varchar(128) DEFAULT NULL,
  `root_run_id` varchar(128) DEFAULT NULL,
  `agent_code` varchar(128) DEFAULT NULL,
  `agent_title` varchar(512) DEFAULT NULL,
  `subagent_session_id` varchar(128) DEFAULT NULL,
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_chat_subagent_run_invocation` (`session_id`, `invocation_id`),
  KEY `idx_chat_subagent_run_session_started` (`session_id`, `started_at`)
) COMMENT='子智能体调用卡片';

DROP TABLE IF EXISTS `chat_subagent_event`;
CREATE TABLE `chat_subagent_event` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `tenant_id` bigint NOT NULL,
  `session_id` bigint NOT NULL,
  `invocation_id` varchar(128) NOT NULL,
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_chat_subagent_event_session_id` (`session_id`, `event_id`),
  UNIQUE KEY `uk_chat_subagent_event_session_sequence` (`session_id`, `invocation_id`, `sequence`),
  KEY `idx_chat_subagent_event_session_invocation_seq` (`session_id`, `invocation_id`, `sequence`)
) COMMENT='子智能体调用过程事件';
